    ...
    match event {
        request_response::Event::Message { peer, message, .. } => match message {
            ...
            request_response::Message::Response { .. } => {
                handle_taliro_response_message_event(peer, message, bus_tx, bus_tx_res_factory)
                    .await
            }
        },
        _ => {}
...
async fn handle_taliro_request_message_event(
    ...
    let response = match request {
        TaliroProtocolRequest::GetBlockchainTip => {
            let (command, res_fut) = bus_tx_res_factory.build_blk_cmd_get_tip_info();
        ...
        TaliroProtocolRequest::GetBlockByHeight(height) => {
            let (command, res_fut) = bus_tx_res_factory.build_blk_cmd_get_block_by_height(height);
            let Ok(_) = bus_tx.send(command).await else {
        ...
        TaliroProtocolRequest::GetBlocksByHeightRange(range) => {
            let (command, res_fut) =
                bus_tx_res_factory.build_blk_cmd_get_blocks_by_height_range(range);
            let Ok(_) = bus_tx.send(command).await else {
...
async fn handle_taliro_response_message_event(
    ...
    match response {
        TaliroProtocolResponse::BlockchainTip(block_info) => {
            let (command, res_fut) =
                bus_tx_res_factory.build_p2p_cmd_receive_blockchain_tip_info(peer_id, block_info);
            let Ok(_) = bus_tx.send(command).await else {
        ...
        TaliroProtocolResponse::GetBlocksByHeightRange(blocks) => {
            ...
            let (command, res_fut) =
                bus_tx_res_factory.build_p2p_cmd_receive_blocks(peer_id, blocks);
            let Ok(_) = bus_tx.send(command).await else {
                log_net_taliro_error!("Failed to send ReceiveBlocksByHeightRange command.");
                return;
            };
